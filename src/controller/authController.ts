...
export const sendOtpController = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        if (!companyName) return res.status(400).json({
            error: "companyName is required"
        })
        if (!companyEmail) return res.status(400).json({
            error: "companyEmail is required"
        })
        if (!employeeSize) return res.status(400).json({
        ...
        if (!phoneOtp) return res.status(500).json({ error: "phone Otp  not send" });
...
export const verifyEmailOtpController = async (req: Request, res: Response): Promise<any> => {
    try {
...
export const verifyPhoneOtpController = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        if (!phoneOtp) return res.status(400).json({
